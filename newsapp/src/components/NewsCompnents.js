...
export default class NewsCompnents extends Component {
  ...
  async componentDidMount() {
    ...
    let data = await fetch(
      `https://newsapi.org/v2/top-headlines?country=in&apiKey=${process.env.API_KEY}&page=1&pageSize=${this.props.pageSize}`
    );
  ...
  prevBtnClick = async () => {
    ...
    this.setState({loading:true})
    let data = await fetch(
      `https://newsapi.org/v2/top-headlines?country=in&apiKey=${process.env.API_KEY}&page=${
        this.state.page - 1
